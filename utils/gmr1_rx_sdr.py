...
# Description: GNU Radio script to receive GSM signals
...
# Use the following import statement for the firdes module
...
class Channel(object):
    ...
    def align_freq(cls, freq):
        ...
        chan = round((freq - base_freq) / cls.BASE_BANDWIDTH)
        return base_freq + chan * cls.BASE_BANDWIDTH
    ...
    for k in pca:
        if k == 'gain':
            for ci, v in (getattr(raw, k) or []):
                pcad.setdefault(ci, AttrDictWithFallback(
                    _fallback=pcad[None])).setdefault(k, {})[v[0]] = v[1]
        else:
            for ci, v in (getattr(raw, k) or []):
                pcad.setdefault(ci, AttrDictWithFallback(
                    _fallback=pcad[None]))[k] = v

        # Gain: Transform to dict and handle fallback mix
    if 'gain' in pcad[None]:
        fgs = pcad[None]['gain']
        for k, v in pcad.items():
            if k is None:
                continue
            if 'gain' not in v:
                continue
            for l, w in fgs.items():
                if l not in v['gain']:
                    v['gain'][l] = w
...
class PFBBase(gr.hier_block2):
    ...
    def __init__(self, center_freq, samp_rate, chan_width, chan_align_fn, need_Nx=False, sps=4):
        ...
        if abs(mid_center_freq - center_freq) > 200:
            self.rotation = 2.0 * math.pi * \
                (self.center_freq - new_center_freq) / samp_rate
        else:
            self.rotation = 0
        ...
        if abs(self.resamp - 1.0) < 1e-5:
            ...
            mid_samp_rate = (math.ceil(samp_rate / chan_width) * chan_width)
        ...
        gr.hier_block2.__init__(self,
                                ...
                                gr.io_signature(
                                    self.n_chans, self.n_chans, gr.sizeof_gr_complex)
                                )
        prev = self
    ...
    def describe(self):
        return '\n'.join([
            "Channelize pre-rotation : %s" % (
                ("%f rad/sample" % self.rotation) if (self.rotation != 0) else "None"),
            "Channelize pre-resample : %s" % (
                ("%f" % self.resamp) if (self.resamp != 1) else "None"),
...
class PFBOutputParameters(object):
    ...
    def __init__(self, width, chan_width, sym_rate, sps):
        ...
        if width > 1:
            ...
            self.rotation = - math.pi * \
                ((self.width-1) / (2.0 * self.width_synth))
    ...
    def describe(self):
        return '\n'.join([
            "Width       : %d" % self.width,
            "Synthesizer : %s" % (("%d chans, %d taps" % (
                self.width_synth, len(self.taps_synth))) if self.width > 1 else "None"),
            "Resampling  : %f [%d taps, 32 filters]" % (
                self.resamp, len(self.taps_resamp)),
...
class DirectOutputParameters(object):
    ...
    def describe(self):
        return '\n'.join([
            ...
            "Resampling rate: %f [%d taps, 32 filters]" % (
                self.resamp, len(self.taps_resamp)),
            "Min Delay : %f\n" % (self.min_delay(),),
        ])
        ...
        if self.resamp != 1:
            if need_rrc:
                self.taps_resamp = firdes.root_raised_cosine(
                    ...
                    self.sym_rate,
                    0.35,
                    int(11.0 * 32 * self.samp_rate /
                        (self.decim1 * self.decim2 * self.sym_rate))
                )
                need_rrc = False
            else:
...
class DirectOutputBranch(gr.hier_block2):
    ...
    def __init__(self, params, freq, filename):
        ...
        # Freq xlating filter
        if params.decim1 > 1:
            self.filt1 = filter.freq_xlating_fir_filter_ccf(
                params.decim1,
                params.taps1,
                freq,
                params.samp_rate
        ...
        if params.decim2 > 1:
            self.filt2 = filter.fir_filter_ccf(
                params.decim2,
                params.taps2
            )
        ...
        #     prev = self.resamp
        if params.resamp != 1:
            taps_resamp = params.taps_resamp if isinstance(
                params.taps_resamp, list) else [params.taps_resamp]
            self.resamp = pfb.arb_resampler_ccf(
                taps=taps_resamp,
                rate=params.resamp,
                flt_size=32
        ...
        self.sink = blocks.file_sink(
            gr.sizeof_gr_complex, f"{filename}", False)
        self.connect((prev, 0), (self.sink, 0))
...
class top_block(gr.top_block):
    def __init__(self, config):
        # Super init
        gr.top_block.__init__(self, "GMR-1 L-band RX Top Block", True)
    ...
    def _setup_qt_channel(self, chan):
        ...
        fblk_win = Qt.QWidget.createWindowContainer(fblk.pyqwidget())
        self.top_layout.addWidget(fblk_win)
        self.connect(self.source_ep[chan], fblk)
    ...
    def _setup_source_channel(self, chan):
        ...
        if gain:
            ...
            for gs, gv in gain.items():
                self.source.set_gain(gv, gs, chan)

        # Time limit or direct
        if self.config.time:
            hb = blocks.head(gr.sizeof_gr_complex, int(
                1.0 * self.source_rate * self.config.time))
    ...
    def _arfcn_prepare(self):
        ...
        for arfcn in self.config.arfcns:
            bs = sorted(range(len(self.source_freq)), key=lambda i: abs(
                arfcn.frequency - self.source_freq[i]))[0]
    ...
    def _setup_direct(self, sps=4):
        ...
        for source_chan, arfcns in self.source_arfcns.items():
            for arfcn in arfcns:
                ...
                if abs(df) >= (self.source_rate / 2):
                    print(
                        f"ARFCN {arfcn} ({eng_notation.num_to_str(f)}Hz) is outside the range\n")
                    continue

                # Debug print
                print(
                    f"ARFCN {arfcn} (abs: {eng_notation.num_to_str(f)}Hz, rel: {eng_notation.num_to_str(df)}Hz)")
    ...
    def _setup_pfb(self, sps=4):
        # Do we need more than 1x channels?
        need_Nx = list(self.needed_widths.keys()) != [1]

        # Create the base channelization block for each source channel
        self.pfb_base = {}
        for source_chan, freq in sorted(self.source_freq.items()):
            ...
            self.connect((self.source, source_chan),
                         (self.pfb_base[source_chan], 0))
        ...
        for source_chan, arfcns in self.source_arfcns.items():
            ...
            term = blocks.null_sink(gr.sizeof_complex_float)
            i = 0
            for index in range(self.pfb_base[source_chan].n_chans):
                if index not in used_indexes:
                    self.connect(
                        (self.pfb_base[source_chan], index), (term, i))
                    i += 1
...
def main():
    ...
    if args.qt:
        # Qt config
        if Qt.QT_VERSION_STR >= "4.5.0":
            Qt.QApplication.setGraphicsSystem(
                gr.prefs().get_string('qtgui', 'style', 'raster'))
    ...
    # Force garbage collection, to clean up Qt widgets
    tb = None
